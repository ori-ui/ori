...
fn properties(input: &DeriveInput) -> TokenStream {
    ...
    quote! {
        const _: () = {
            ...
            impl #ily_core::Properties for #name {
...
fn bindings(input: &DeriveInput) -> TokenStream {
    ...
    quote! {
        const _: () = {
            ...
            impl #ily_core::Bindings for #name {
                type Setter<'a> = Setter<'a>;
    ...
    let fields = fields.named.iter().filter_map(|field| {
        ...
        Some(quote! {
            pub fn #event<'b>(
                &mut self,
                cx: #ily_core::Scope<'b>,
                #name: &'b #ily_core::Signal<<#ty as #ily_core::Bindable>::Item>
            ) {
                <#ty as #ily_core::Bindable>::bind(&mut self.this.#name, cx, #name);
            }
        })
    });
