...
use ily_core::Vec2;
use ily_graphics::{Frame, Mesh, Vertex};
use wgpu::{
    include_wgsl,
    ...
    FragmentState, IndexFormat, PipelineLayoutDescriptor, Queue, RenderPass, RenderPipeline,
    RenderPipelineDescriptor, ShaderStages, TextureFormat, VertexBufferLayout, VertexStepMode,
};
...
impl MeshInstance {
    pub fn new(device: &Device, queue: &Queue, pipeline: &MeshPipeline, mesh: &Mesh) -> Self {
        ...
        let vertex_buffer = Self::create_vertex_buffer(device, queue, mesh.vertex_bytes());
        let index_buffer = Self::create_index_buffer(device, queue, mesh.index_bytes());
    ...
    fn update_vertex_buffer(&mut self, device: &Device, queue: &Queue, mesh: &Mesh) {
        ...
        if self.vertex_buffer.size() < data.len() as u64 {
            self.vertex_buffer = Self::create_vertex_buffer(device, queue, data);
        } else {
    ...
    fn update_index_buffer(&mut self, device: &Device, queue: &Queue, mesh: &Mesh) {
        ...
        if self.index_buffer.size() < data.len() as u64 {
            self.index_buffer = Self::create_index_buffer(device, queue, data);
        } else {
            queue.write_buffer(&self.index_buffer, 0, data);
        }
    }

    fn create_vertex_buffer(device: &Device, queue: &Queue, contents: &[u8]) -> Buffer {
    ...
    fn create_index_buffer(device: &Device, queue: &Queue, contents: &[u8]) -> Buffer {
        device.create_buffer_init(&BufferInitDescriptor {
...
impl MeshPipeline {
    ...
    pub fn prepare_mesh(&mut self, device: &Device, queue: &Queue, mesh: &Mesh, index: usize) {
        if let Some(instance) = self.instances.get_mut(index) {
            ...
            let instance = MeshInstance::new(device, queue, self, mesh);
