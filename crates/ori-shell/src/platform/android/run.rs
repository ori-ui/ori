...
use super::{AndroidError, ANDROID_APP};
...
fn show_soft_input(app: &AndroidApp, show: bool) {
    let vm = match unsafe { JavaVM::from_raw(app.vm_as_ptr() as _) } {
        Ok(vm) => vm,
        Err(err) => {
            println!("Failed to get JavaVM: {:?}", err);
            return;
        }
    };
    ...
    let mut env = match vm.attach_current_thread() {
        Ok(env) => env,
        Err(err) => {
            println!("Failed to attach current thread: {:?}", err);
            return;
        }
    };
    ...
    let class_cx = match env.find_class("android/content/Context") {
        Ok(class) => class,
        Err(err) => {
            println!("Failed to find class: {:?}", err);
            return;
        }
    };
    ...
    let ims = match env.get_static_field(class_cx, "INPUT_METHOD_SERVICE", "Ljava/lang/String;") {
        Ok(ims) => ims,
        Err(err) => {
            println!("Failed to get field: {:?}", err);
            return;
        }
    };
        ...
        .l()
    {
        ...
        Err(err) => {
            println!("Failed to get input manager: {:?}", err);
            return;
        }
    };
        ...
        .l()
    {
        ...
        Err(err) => {
            println!("Failed to get window: {:?}", err);
            return;
        }
    };
        ...
        .l()
    {
        Ok(view) => view,
        Err(err) => {
            println!("Failed to get view: {:?}", err);
            return;
        }
    };
    ...
    if show {
        ...
        if !result {
            println!("Failed to show soft input");
        }
    } else {
        ...
        if !result {
            println!("Failed to hide soft input");
        }
    }
}
