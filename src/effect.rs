...
/// A [`View`] that has no [`View::Element`] and can therefore only produce side-effects.
///
/// Implemented for all [`View`]s with an element of `()`.
pub trait Effect<C, T>: View<C, T, Element = ()> {}

/// A sequence of [`Effect`]s.
///
/// Implemented for all [`ViewSeq`]s with an element of `()`.
